...
Do not use it for an authorized post-build assessment or as a vulnerability scanner for an existing codebase; use an authorized scanning tool or specialist security assessment. When formalizing the resulting requirements, use the repository's normal specification workflow and make completion claims only from direct verification evidence.

## When not to use

End-to-end tenant models, control/application-plane placement, commercial lifecycle, capacity planning, and SaaS operating architecture are outside this skill; it owns the threat model and enforceable isolation controls. Route privacy lifecycle and deletion-verification artifacts to the project's privacy owner.
...
| Preparing artifacts, exceptions, rollback, or response for release | [references/release-evidence.md](references/release-evidence.md); carry those controls and evidence through the project's build, registry, promotion, and deployment gates |
